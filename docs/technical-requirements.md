...
- The visual design is a translucent, layered interface and lives entirely in the single vendored `style.css`: translucent layered surfaces (`backdrop-filter` blur over a fixed gradient backdrop), depth via soft shadows, and a light/dark palette switched by `prefers-color-scheme` through CSS custom properties. The direction is adapted to sharp edges (square corners, `border-radius: 0`) so controls read as crisp rather than rounded. Shared visual values (colors, materials, borders, radii, shadows, spacing, motion timings) are declared once as CSS custom properties and referenced throughout rather than repeated as literals. No CSS framework or build step is introduced. The full visual direction is specified in `design-requirements.md`.
