...
The language token comes from the detection step when it reached a confident
decision (``WorkItem.language``); otherwise the token already in the filename is
preserved as-is. To avoid silently dropping a meaningful but unrecognised suffix, a
subtitle with no recognised language token whose basename does not resemble the
video's basename is left unchanged with a warning rather than collapsed onto the bare
video name.
"""
...
def normalize_filename(item: WorkItem, _config: Config) -> None:
    ...
    base, file_language, flags = split_subtitle_name(item.source)
    language = item.language if item.language is not None else file_language
